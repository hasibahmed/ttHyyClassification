...
def roc(y_true, y_preds, labels=None, legend=True, ax=None):
    ...
    if labels is None:
        ...
        if isinstance(labels, str):
            labels = [labels]
            pass
        assert len(labels) == N, "[roc] Number of predictions ({}) and associated labels ({}) do not match.".format(
            N, len(labels))
        pass
...
def profile(m, ys, labels=None, bins=40, ax=None):
    ...
    assert len(labels) == N, "[profile] Number of observables ({}) and associated labels ({}) do not match.".format(
        N, len(labels))
...
def sculpting(m, y, preds, labels=None, effsig=0.5, bins=40, ax=None):
    ...
    assert len(labels) == N, "[sculpting] Number of observables ({}) and associated labels ({}) do not match.".format(
        N, len(labels))
    ...
    ax.hist(m[sig], color='red',  bins=bins, weights=wsig,
            label='Signal, incl.', histtype='step', lw=2)
    ax.hist(m[~sig], color='blue', bins=bins, weights=wbkg,
            label='Background, incl.', histtype='step', lw=2)
    ax.hist([0], weights=[0], color='black',
            label='Bkgds., $\\varepsilon_{{sig}} = {:.0f}\%$ cut'.format(effsig * 100.), **common)
    ...
    for pred, label in zip(preds, labels):
        ...
        # Assuming signal towards larger values
        msk = (~sig) & (pred > cut).ravel()
...
def posterior(adv, m, z, nb_bins=100, title=''):
    ...
    ax[0].hist(m, bins, density=1., alpha=0.3,
               color='black', label='Background, incl.')
    for col, z_ in zip(colours, zs):
        ...
        ax[0].plot(mt_pdf * scale, posterior / scale,
                   color=col, label='z = {:.1f}'.format(z_))
        pass

    # -- Right pane
    ax[1].hist([0], bins, alpha=0.3, weights=[0],
               color='black', label='Background, $z_{NN}$-bin.')
    for col, z_ in zip(colours, zs):
        # Draw background jet distribution for classifier output `z`
        msk = np.abs(z - z_).ravel() < tol

        ax[1].hist(m[msk], bins, color=col, density=1., alpha=0.3,
                   label='  {:.2f} < $z_{{NN}}$ < {:.2f}'.format(z_ - tol, z_ + tol))

        # Draw adversary posterior p.d.f. for classifier output `z`
        posterior = adv.predict([z_*np.ones_like(mt_pdf), mt_pdf])
        ax[1].plot(mt_pdf * scale, posterior / scale,
                   color=col, label='z = {:.1f}'.format(z_))
        pass
